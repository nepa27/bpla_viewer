// components/BrushableBarChart/BrushableBarChart.jsx
import { axisBottom, axisLeft, brushX, max, scaleBand, scaleLinear, select, timeFormat } from 'd3';
...
export const BrushableBarChart = memo(({ data, onBrush }) => {
  ...
  useEffect(() => {
    ...
    // Группируем данные по месяцам правильно
    const groupedData = {};

    data.forEach((item) => {
      const date = new Date(item.date);
      // Создаем ключ в формате "YYYY-MM" для правильной группировки
      const monthKey = `${date.getFullYear()}-${String(date.getMonth() + 1).padStart(2, '0')}`;

      if (!groupedData[monthKey]) {
        groupedData[monthKey] = {
          date: new Date(date.getFullYear(), date.getMonth(), 1), // Первый день месяца
          count: 0,
        };
      }
    ...
    const monthlyData = Object.values(groupedData).sort((a, b) => a.date - b.date);
    ...
    // Шкалы с использованием месячных данных
    const x = scaleBand()
      .domain(monthlyData.map((d) => d.date.getTime()))
      .range([0, width])
      .padding(isMobile ? 0.1 : 0.2);

    const y = scaleLinear()
      .domain([0, max(monthlyData, (d) => d.count)])
      .nice()
      .range([height, 0]);
    ...
    // Рендерим оси
    AxisBottom(g, x, height, width, true); // передаем флаг для форматирования месяцев
    AxisLeft(g, y, width);
    ...
    // Рендерим столбцы с месячными данными
    Marks(g, monthlyData, x, y, height);
    ...
    function brushed(event) {
      ...
      const dates = monthlyData.map((d) => d.date.getTime());
      ...
      // Расширяем диапазон до конца месяца для более точной фильтрации
