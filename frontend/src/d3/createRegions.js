...
export const createRegions = ({svg, mapGroup, dataToRender, path, setSelectedRegion, isSingleRegion, setTooltip, height}) => {
    
    const regions = mapGroup.selectAll("path.region")
          .data(
          ...
          .on("mouseover", function(event, d) {
            ...
            setTooltip({
              visible: true,
              content: d.properties?.region || d.properties?.name || d.properties?.['hc-key'] || 'Неизвестный регион',
              x: x,
              y: y
            });
          })
